...
# значит «запись уже лежит в сообществе» – Click в час выхода не нужен. Оба
# канала Телеграма теперь здесь же: их отложку Click ставит браузером через
# веб-версию, и держит её сам Телеграм (раньше ТГ шёл ботом, и время держал
# Click – отсюда прежняя обратная пометка).
#
...
SELF_HOSTED = ("vk", "ok", "zen", "tg-staff", "tg-client")
