...
2. Абзац благодарности – РОВНО ТРИ коротких предложения, не больше. Тон тёплый и живой, без канцелярита.
Первое: «Благодарим вас за отзыв и доверие к нашей компании!» или «Благодарим за отзыв и рекомендацию!».
Второе: назвать своими словами то конкретное, о чём написал клиент – что именно он купил, в каком объёме, что ему понравилось. Не общими словами «оценили качество», а по существу отзыва: «заказом 12 тонн арматуры», «покупкой профильной трубы и сетки-рабицы», «выгодными ценами на металлопрокат», «чёткостью и своевременностью доставки».
Третье – одно из двух, смотря по отзыву:
– если в отзыве есть замечание (долгие сроки, ожидание поставки и т.п.) – поблагодарить за обратную связь и сказать, что работаем над этим. Не оправдываться и ничего не обещать сверх этого. Пожелание о сотрудничестве в этом случае НЕ добавлять;
– если замечаний нет – «Будем рады новому сотрудничеству!».

ДЛИНА – это важно. Ассортиментная фраза из пункта 3 занимает около 140 знаков. Абзац благодарности должен быть примерно таким же: 150-230 знаков. Больше 250 знаков – уже плохо, абзацы становятся разной величины и ответ выглядит неровным. Лучше убрать подробность, чем растянуть абзац.
...
Оба абзаца должны выглядеть примерно одинаково по величине – ответы на разные отзывы обязаны быть похожими.
...
"SMU": """Напиши ответ на отзыв от лица компании по продаже металлопроката Стальметурал. Тон – деловой, простой, спокойный, без пафоса; предложения естественно связаны между собой, как будто пишет обычный сотрудник, а не рекламный копирайтер.
Текст отзыва: [вставить отзыв] Автор отзыва: [имя]
Структура – строго 3 абзаца, разделённых пустыми строками, БЕЗ дополнительных абзацев:
1. Обращение: «Уважаемая»/«Уважаемый» (род определяй по имени) + имя – одно предложение, без слова «спасибо» здесь. Пример: «Уважаемая Гульмира!»
2. Основная часть – РОВНО ТРИ предложения, не больше:
   • Предложение 1: благодарность за отзыв + пересказ своими словами того, что клиент отметил – все детали из отзыва в ОДНОМ предложении (перечисли через запятую и «а также»), не растягивай на два-три предложения.
   • Предложение 2: короткое пожелание дальнейшего сотрудничества.
   • Предложение 3: ассортиментная фраза дословно: «У нас постоянно в наличии лист г/к, нержавейка, трубы сварные и бесшовные, цветной металлопрокат, сортовой прокат, балки, арматура.»
3. Закрытие: «С уважением, команда Стальметурал.»
Запрещено: усилительные слова и обороты («очень», «искренне», «огромное спасибо за то, что...», «отлично», «с нетерпением ждём»); ассортиментная фраза отдельным абзацем (она – последнее предложение основного абзаца, не «довесок»); любой абзац сверх трёх, включая рассуждения про «широкий выбор», «экономию времени», «максимальный объём позиций» и подобные рекламные обороты.
...
Пример готового ответа – на ДРУГОЙ отзыв, дословно не копировать, использовать как образец тона и структуры:
Уважаемая Гульмира!

Благодарим вас за отзыв – особенно ценно, что вы оценили качество и прочность шестигранника, доступные цены на инструменты, а также приветливость и отзывчивость наших консультантов. Будем рады новому долгосрочному и плодотворному сотрудничеству. У нас постоянно в наличии лист г/к, нержавейка, трубы сварные и бесшовные, цветной металлопрокат, сортовой прокат, балки, арматура.

С уважением, команда Стальметурал.""",

"MPE": """Напиши ответ на отзыв от лица компании по продаже металлопроката Метпромэнерго. Тон – деловой, простой, спокойный, без пафоса; предложения естественно связаны между собой, как будто пишет обычный сотрудник, а не рекламный копирайтер.
Текст отзыва: [вставить отзыв] Автор отзыва: [имя]
Структура – строго 3 абзаца, разделённых пустыми строками:
1. Обращение по имени (только имя, без «Уважаемый/Уважаемая») + краткая благодарность – одно предложение. Пример: «Иван, спасибо за отзыв!»
2. Основная часть – РОВНО ТРИ предложения в такой последовательности:
   • Предложение 1: перескажи своими словами то, что клиент отметил в отзыве – спокойно и просто, без превращения в рекламный слоган. Пример хорошего пересказа: «машину подали вовремя, документы были готовы» → «вы оценили оперативную подачу транспорта и своевременную подготовку документов».
   • Предложение 2: коротко вырази понимание ситуации клиента или ценность этого для него. Должно логично вытекать из первого.
   • Предложение 3: ассортиментная фраза дословно: «Наш ассортимент закрывает большинство потребностей клиентов: цветной металл, нержавейка, сортовой и листовой прокат, трубная продукция, арматура.»
Запрещено: усилительные слова («очень», «крайне», «максимально», «чрезвычайно», «абсолютно»); двойные и тройные эпитеты подряд («чёткую логистику», «абсолютную готовность», «критическую важность»); канцелярит и рекламные обороты («нам очень приятно», «делаем всё возможное», «минимизировать простои», «оптимизировать расходы»). Каждое предложение – с одним простым определением, максимум.
3. Закрытие: «С уважением, команда Метпромэнерго.»
...
Примеры готовых ответов – на ДРУГИЕ отзывы, дословно не копировать, использовать как образец тона и структуры:

Пример 1:
Иван, спасибо за отзыв!

Рады, что вы оценили оперативную подачу транспорта и своевременную подготовку документов при поставке бериллия. Мы понимаем, как важна каждая минута при разгрузке, и всегда стремимся беречь время и бюджет наших клиентов. Наш ассортимент закрывает большинство потребностей клиентов: цветной металл, нержавейка, сортовой и листовой прокат, трубная продукция, арматура.
...
Приятно, что вы оценили нашу работу в сравнении с другими поставщиками, а также остались довольны качеством ниобия и быстрой доставкой. Будем рады новому сотрудничеству. Наш ассортимент закрывает большинство потребностей клиентов: цветной металл, нержавейка, сортовой и листовой прокат, трубная продукция, арматура.

С уважением, команда Метпромэнерго.""",

"MPI": """Напиши ответ на отзыв от лица компании по продаже металлопроката МетПромИнтекс.
Текст отзыва: [вставить отзыв] Автор отзыва: [имя]

Обращения по имени у этого бренда нет вовсе: ни «Уважаемый», ни имени клиента
в ответе быть не должно. Тон простой и тёплый, но ДЕЛОВОЙ: без канцелярита и
без разговорных словечек.

Отвечает компания, а не приятель. Слова из отзыва не копируй – пересказывай
их деловым языком, суть при этом сохраняется:
– «классный выбор», «крутой выбор», «выбор реально большой» – это «широкий
выбор»;
– «ценники», «ценники норм», «по цене норм» – это «адекватные цены» или
«доступные цены»;
– «быстро привезли», «доставка была быстрая» – это «оперативная доставка»;
– «всё супер», «всё огонь», «шикарно» – это «всё прошло хорошо».
Отзыв «Здесь очень классный выбор металла. Ценники более, чем адекватные.
Доставка была быстрая.» разворачивается в «Приятно слышать, что вы оценили
широкий выбор металла, адекватные цены и оперативную доставку.» – про то же
самое, но словами компании. Написать «вы отметили классный выбор металла и
классные ценники» – это ошибка.

Структура – ровно три части, между ними ПУСТАЯ СТРОКА. Первая строка берётся
из двух готовых, последняя не меняется никогда:

1. Первая строка – одна из двух, смотря по отзыву:
– клиент нас РЕКОМЕНДУЕТ или советует другим («рекомендую», «однозначно
рекомендую», «советую всем», «буду рекомендовать») – дословно:
«Спасибо за отзыв и рекомендацию!»;
– про рекомендацию в отзыве НЕТ НИ СЛОВА (просто похвалили выбор, цены,
доставку, работу менеджера) – дословно: «Спасибо за отзыв!»
Благодарить за рекомендацию, которой человек не давал, нельзя. «Хороший выбор
металла, цены адекватные, доставка быстрая» – это не рекомендация, тут первая
строка «Спасибо за отзыв!».
Своих вариантов не придумывать: «Спасибо за положительный отзыв!», «Спасибо,
что поделились своим мнением!» – это ошибка, первая строка всегда одна из
двух приведённых.

2. Абзац благодарности – 2-4 предложения, одним абзацем.
Начинается словами «Приятно слышать, что …» – и сразу по сути отзыва.
Оборот полный, обрезать нельзя: не «Приятно, что …» и не «Нам приятно, что …»,
а именно «Приятно слышать, что …».
Второй раз благодарить за отзыв НЕЛЬЗЯ: «Благодарим вас за отзыв!»,
«Благодарим за отзыв и доверие», «Спасибо за отзыв» в начале абзаца – это
масло масляное, за отзыв уже сказано спасибо первой строкой.
Дальше 1-2 предложения о том КОНКРЕТНОМ, что написал клиент: что купил, какой
объём, что именно ему понравилось, что он отметил. Своими словами, по существу
отзыва, а не общими словами «оценили качество». Своими словами – это деловым
языком компании, а не разговорными словами клиента: см. правило про тон выше.
Заканчивается абзац ДОСЛОВНО этим предложением: «Обращайтесь к нам ещё, всегда
готовы предложить широкий ассортимент металлопроката и индивидуальный подход
к каждому клиенту.» Оно стоит в конце ЭТОГО ЖЕ абзаца, не отдельной строкой,
и не меняется никогда.

3. Последняя строка – дословно: «С уважением, МетПромИнтекс.»

Ограничения:
Не обращаться по имени вовсе
Не писать «и рекомендацию», если клиент нас никому не рекомендовал
Не повторять разговорные слова клиента: «классный», «крутой», «шикарный»,
«супер», «норм», «ценники» – в ответе компании их быть не должно
Не использовать слово «очень» вообще. Не «Нам очень приятно, что…», а
«Приятно слышать, что…». Не «Очень рады, что…», а «Мы рады, что…».
...
«МетПромИнтекс» – 1 раз, только в последней строке
Не заканчивать ответ фразами вроде «Будем рады видеть вас снова!» – последним
предложением абзаца всегда идёт фраза про ассортимент

Образцы – это ГОТОВЫЕ ответы заказчика на другие отзывы. Держись их вида
дословно: та же последняя строка, тот же зачин абзаца, та же фраза в конце
абзаца, а первая строка – по правилу выше. Меняется только середина – она про
конкретный отзыв. В первом образце клиент пишет «Рекомендую» – там первая
строка с рекомендацией; в остальных рекомендации нет – там просто «Спасибо за
отзыв!». Обрати внимание и на слова: в отзывах они разговорные, в ответах –
деловые.

Отзыв: «Купил гладкую арматуру по достаточно выгодной цене. Так что остался доволен. Рекомендую.»
Ответ:
Спасибо за отзыв и рекомендацию!

Приятно слышать, что вы остались довольны покупкой гладкой арматуры по выгодной цене и рекомендуете нас. Обращайтесь к нам ещё, всегда готовы предложить широкий ассортимент металлопроката и индивидуальный подход к каждому клиенту.

С уважением, МетПромИнтекс.

Отзыв: «Склад отличный, выбор реально достаточно большой. Всегда беру арматуру только здесь. По цене норм.»
Ответ:
Спасибо за отзыв!

Приятно слышать, что вы оценили большой выбор продукции и доступные цены на арматуру. Обращайтесь к нам ещё, всегда готовы предложить широкий ассортимент металлопроката и индивидуальный подход к каждому клиенту.

С уважением, МетПромИнтекс.

Отзыв: «Я уже давно заказываю металлопрокат на этой металлобазе и всегда остаюсь доволен качеством продукции и уровнем сервиса. Впечатляет ассортимент товаров – здесь есть всё, что может только понадобиться. Отдельно хочется отметить наличие нужного мне количества гладкой арматуры. Что касается цены, то она здесь вполне доступная, а при крупных заказах предоставляется неплохая скидка.»
Ответ:
Спасибо за отзыв!

Приятно слышать, что вы уже долгое время остаётесь довольны качеством продукции, уровнем сервиса и широким ассортиментом. Рады, что гладкая арматура всегда есть в наличии в нужном вам объёме, а наши цены и система скидок на крупные заказы делают сотрудничество ещё приятнее. Обращайтесь к нам ещё, всегда готовы предложить широкий ассортимент металлопроката и индивидуальный подход к каждому клиенту.

С уважением, МетПромИнтекс.

Отзыв: «Здесь очень классный выбор металла. Ценники более, чем адекватные. Доставка была быстрая.»
Ответ:
Спасибо за отзыв!

Приятно слышать, что вы оценили широкий выбор металла, адекватные цены и оперативную доставку. Обращайтесь к нам ещё, всегда готовы предложить широкий ассортимент металлопроката и индивидуальный подход к каждому клиенту.
...
REVIEW_COMMON_RULES = """
Общие правила – они важнее любых примеров выше:

1. Тире. Длинное тире не использовать НИКОГДА, только короткое «–». В
диапазонах («10–12 тонн») – дефис или короткое тире. Если в примере выше
встретилось длинное тире, всё равно пиши короткое.
...
женский;
– если вместо имени передано слово «Клиент» – пиши ровно «Уважаемый клиент!»,
со строчной буквы и без имени.

3. Назови то, что клиент купил. Если в отзыве упомянут товар, изделие или
услуга – труба, арматура, швеллер, лист, сетка, резка, доставка, объём в
тоннах – это ОБЯЗАТЕЛЬНО должно прозвучать в ответе своими словами: «рады,
что труба для забора подошла», «довольны заказом 12 тонн арматуры». Общие
слова «оценили качество нашей продукции» вместо конкретики – это ошибка.
Если же в отзыве конкретики нет (написали только «всё хорошо», «отличная
компания»), ничего не выдумывай и отвечай общими словами.

{assortment_rule}

5. «Вы», «вас», «ваш» – со строчной буквы. С прописной («Вы») не писать.

6. Никакой разметки: без звёздочек, решёток, markdown и кавычек вокруг
всего ответа. Только готовый текст, который можно вставить как есть.
"""

# Правило про ассортиментную фразу у МПИ своё. У четырёх брендов она стоит
# отдельным абзацем, а у МПИ – последним предложением абзаца благодарности:
# так написаны образцы заказчика. Общее правило «отдельный абзац и ничего
# кроме неё» уводило модель ровно от того вида, который она ждёт.
REVIEW_ASSORTMENT_RULE = {
    "": "4. Ассортиментная фраза – отдельный абзац и НИЧЕГО КРОМЕ НЕЁ. Не дописывай\n"
        "к ней ни предложения: этот абзац у всех ответов обязан быть одинаковым.",
    "MPI": "4. Фраза «Обращайтесь к нам ещё, всегда готовы предложить широкий ассортимент\n"
           "металлопроката и индивидуальный подход к каждому клиенту.» – это ПОСЛЕДНЕЕ\n"
           "ПРЕДЛОЖЕНИЕ абзаца благодарности, слово в слово. Не выносить её отдельным\n"
           "абзацем, не менять формулировку и не заканчивать ответ чем-то другим\n"
           "(«Будем рады видеть вас снова!» – ошибка).",
}


def review_common_rules(project_id: str = "") -> str:
    """Общие правила с подставленным правилом про ассортиментную фразу."""
    rule = REVIEW_ASSORTMENT_RULE.get(project_id) or REVIEW_ASSORTMENT_RULE[""]
    return REVIEW_COMMON_RULES.replace("{assortment_rule}", rule)

# Вид ответа МПИ: первая строка, последнее предложение абзаца благодарности и
# подпись у всех ответов одинаковы – так написаны образцы заказчика. Держим их
# здесь, чтобы код мог поправить форму, если модель от неё уйдёт: в боевом
# прогоне она то меняла первую строку («Спасибо, что поделились своим
# мнением!»), то теряла фразу про ассортимент и заканчивала ответ своим
# «Будем рады видеть вас снова!».
#
# Первых строк две. Заказчик: благодарить за рекомендацию можно только тогда,
# когда клиент нас и правда кому-то рекомендовал. Отзыв «классный выбор металла,
# ценники адекватные, доставка быстрая» рекомендацией не является – на такой
# ответ начинается просто «Спасибо за отзыв!».
REVIEW_MPI_OPENER = "Спасибо за отзыв и рекомендацию!"
REVIEW_MPI_OPENER_PLAIN = "Спасибо за отзыв!"
REVIEW_MPI_TAIL = ("Обращайтесь к нам ещё, всегда готовы предложить широкий ассортимент "
                   "металлопроката и индивидуальный подход к каждому клиенту.")
REVIEW_MPI_SIGN = "С уважением, МетПромИнтекс."
...
# Разговорные слова. Запрета на них в промптах нет, но в ответе ОТ ЛИЦА
# КОМПАНИИ они звучат панибратски: заказчик просила отвечать по-деловому.
# Берутся они из самого отзыва – модель переносит слова клиента дословно
# («вы отметили классный выбор металла и классные ценники»).
#
# Ходовые сочетания код чинит сам (см. _MPI_CASUAL в reviews.py), а этот
# список – для предупреждения человеку: заменить слово в общем случае нельзя,
# не зная, о чём речь. Корни, а не слова целиком: «классн» – это и «классный»,
# и «классные». «Норм» в список не берём: он поймает «нормам ГОСТ».
REVIEW_CASUAL_WORDS = ("классн", "крут", "шикарн", "супер", "клёв", "клев",
                       "прикольн", "офигенн", "ценник")

# Обращения по имени у МПИ нет вовсе – так написан его промпт.
REVIEW_NO_NAME_PROJECTS = ("MPI",)

# Ассортиментная фраза бренда. Она вставляется в ответ ДОСЛОВНО и отдельным
# абзацем – это требование заказчика по всем проектам.
#
# Зачем держать её здесь отдельно, если она и так есть в промпте. Затем, что
# «дословно» модель соблюдает не всегда. В боевом прогоне один ответ ушёл в
# Яндекс со строкой «трубы, armatura – собираем и отгружаем без задержек»:
# слово внезапно оказалось латиницей. Заметить такое глазами в трёх десятках
# ответов невозможно, а сплошной запрет латиницы поставить нельзя – в
# металлоторговле законно встречаются AISI 304, DIN, A2.
#
# Поэтому проверяем не латиницу вообще, а именно эту фразу: она известна
# заранее и меняться не должна. Похожий на неё абзац заменяется на эталонный.
REVIEW_ASSORTMENT = {
    "APS": "Полный набор металлопродукции: цветной прокат, нержавейка, сортовые и "
           "листовые виды, трубы, арматура – собираем и отгружаем без задержек.",
    "IMP": "В нашем сортаменте всегда в наличии цветной металлопрокат, нержавейка, "
           "сортовой прокат, листы г/к, трубы, арматура и другие позиции",
    "SMU": "У нас постоянно в наличии лист г/к, нержавейка, трубы сварные и бесшовные, "
           "цветной металлопрокат, сортовой прокат, балки, арматура.",
