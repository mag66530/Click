...
_PLACE_PREFIX_RX = re.compile(
    ...
    r"городской\s+акимат|районный\s+акимат|акимат|"
...
#
# Слово ищем в ЛЮБОМ месте куска, а не только в начале: Яндекс пишет
# «Тамбовская улица», а не «улица Тамбовская», и такой кусок проходил насквозь.
# И конец слова проверяем не через \b: после «корп.» стоит пробел, а между
# точкой и пробелом границы слова нет – «корп. 2» и «стр. 1» тоже пролезали.
# На живом адресе из Красноярска это давало «город» = «стр. 1».
_NOT_A_PLACE_RX = re.compile(
    r"(?:^|\s)(?:улица|ул\.|проспект|просп\.|пр-?кт|переулок|пер\.|шоссе|бульвар|б-р|"
    ...
    r"помещение|здание|владение|км|территория|промзона|"
    r"промышленная\s+зона|промышленный\s+район)(?:\s|\.|$)",
    re.I,
)

# Номер дома: «5», «116а», «34/в», «33/3», «12 к2». Городом не бывает.
_HOUSE_RX = re.compile(r"\d+\s*[а-яa-z]?(?:\s*[/-]\s*\d*\s*[а-яa-z]?)*", re.I)
...
_REGION_RX = re.compile(
    ...
    r"\bрайон\b|\bулус\b|\bобл\.|\bкр\.|сельский\s+округ|аульный\s+округ)",
    re.I,
)
...
def address_places(address: str) -> set[str]:
    ...
    for raw in str(address or "").split(","):
        part = raw.strip()
        if not part or _NOT_A_PLACE_RX.search(part) or _REGION_RX.search(part):
            continue
        if re.fullmatch(r"[\d\W]+", part) or _HOUSE_RX.fullmatch(norm_text(part)):
            continue
        place = norm_city(part)
        if place and not _HOUSE_RX.fullmatch(place):
            out.add(place)
    return out
...
def compare(item: dict, companies: list[dict], all_ids: set[str] | None = None) -> dict:
    """
    Сравнить строку КП с найденными организациями. Ничего не меняет.

    all_ids – номера ВСЕХ прочитанных организаций аккаунта. Нужны, чтобы
    отличить две разные беды: ссылка в КП ведёт на чужую карточку (та есть
    в аккаунте, но привязана к другому городу) или карточки по ссылке нет
    среди прочитанных вовсе – список организаций устарел либо карточка
    заведена в другом аккаунте.
    """
    sites, emails, phones = [], [], []
    for co in companies:
    ...
    if kp_id and ya_ids and knows_old_ids and kp_id not in ya_ids:
        # Карточка по ссылке есть в аккаунте, но села на другой город – это
        # действительно неверная ссылка. А вот если её нет среди прочитанных
        # вовсе, дело не в ссылке: список организаций устарел (карточку
        # завели после сбора) либо она в другом аккаунте. Заказчик как раз
        # на это и наткнулась: в Яндексе у города две карточки, а в отчёте
        # одна и «ссылка ведёт на другую» – неправда, вводящая в заблуждение.
        if all_ids is not None and kp_id not in all_ids:
            problems.append(f"карточки из ссылки в КП ({kp_id}) нет среди прочитанных "
                            "организаций – перечитайте список: её могли завести позже")
        else:
...
def guess_city(address: str) -> str:
    ...
    Ярославль, Ярославль» → «Ярославль».

    Главная подсказка – повтор: Яндекс называет город дважды, сначала
    округом («городской округ Красноярск»), потом сам по себе. Что
    повторилось, то и город. Это надёжнее, чем брать последнюю подходящую
    часть: улица без слова «улица» («Ленина, 1») от города по написанию
    неотличима, и последней оказывалась она.

    Не повторилось – берём последнюю: в «…, посёлок Заречье, Заречье, …»
    точнее именно она, а не округ в начале.
    """
    кандидаты: list[str] = []
    for raw in str(address or "").split(","):
        part = raw.strip()
        ...
        if re.fullmatch(r"[\d\W]+", part) or _HOUSE_RX.fullmatch(norm_text(part)):
            continue
        кандидаты.append(_PLACE_PREFIX_RX.sub("", part).strip(" .,"))
    if not кандидаты:
        return ""
    сколько: dict[str, int] = {}
    for c in кандидаты:
        ключ = norm_city(c)
        сколько[ключ] = сколько.get(ключ, 0) + 1
    for c in кандидаты:
        if сколько[norm_city(c)] >= 2:
            return c
    return кандидаты[-1]
...
DIFF_HEADERS = ["Страна", "Город", "Что расходится", "В КП", "В Яндексе",
                "Ссылка на карточку"]
NOLINK_HEADERS = ["Страна", "Город", "Название в Яндексе", "Адрес в Яндексе",
                  "Ссылка на карточку"]
...
def nolink_rows(result: dict) -> list[list[str]]:
    """
    Города, где карточка нашлась, а ссылки на неё в КП нет.

    Это не ошибка Яндекса и не расхождение, а дырка в самом КП: ссылку надо
    вписать. Раньше об этом говорила только пометка в строке «Сверки» да
    цифра на дашборде – заказчик их видела, но собрать все такие города
    в одном месте было негде, приходилось идти по строкам глазами.

    Строка на город, а не на карточку: где карточек несколько, это уже
    вопрос листа «Дубли», и сначала надо выбрать нужную.
    """
    out = [list(NOLINK_HEADERS)]
    for it in result.get("items") or []:
        cos = it.get("companies") or []
        if not cos or company_id_from_url(it.get("link", "")):
            continue
        co = cos[0]
        name = co.get("name", "")
        if len(cos) > 1:
            name = f"{name} (карточек: {len(cos)} – см. лист «Дубли»)"
        out.append([it.get("country", ""), it.get("city", ""), name,
                    co.get("address", ""), card_url(str(co.get("id") or ""))])
    return out
...
LEGEND = [
    ...
    ("Нет ссылки в КП – город в КП есть и карточка нашлась, но ссылки на неё "
     "в КП не записано. Скопируйте её из отчёта: со следующей сверки город "
     "будет проверяться по ссылке, а не по названию.", False),
    ("", False),
    ("Если в отчёте меньше карточек, чем видно в Яндексе", True),
    ("Сверка знает только те организации, что прочитаны в последний раз – время "
     "сбора написано на этом же листе сверху. Карточку, заведённую позже, она "
     "не увидит: нажмите «Прочитать организации в Яндексе» заново. Признак "
     "устаревшего списка – пометка «карточки из ссылки в КП нет среди "
     "прочитанных организаций».", False),
    ("Дубли – города, где карточек больше одной. По строке на карточку. Сюда "
     "попадают и города, которых в КП нет: два Красноярска в «Организациях» – "
...
def to_xlsx(rows: list[list[str]], result: dict, sheet_name: str = "Сверка",
    ...
    _list_sheet(wb, "Нет ссылки в КП", nolink_rows(result), link_col=5,
                link_label="Открыть карточку",
                note="Карточка в Яндексе нашлась, а ссылки на неё в КП нет. "
                     "Скопируйте ссылку в КП – со следующей сверки город будет "
                     "проверяться по ней, а не по названию.",
                empty_note="У всех городов ссылка в КП проставлена.")
    _list_sheet(wb, "Дубли", double_rows(result), link_col=9, link_label="Открыть карточку",
                note="Города, где карточек больше одной. Строк столько же, сколько карточек. "
                     "Города, которых в КП нет, тоже здесь – с пометкой в колонке «Страна»: "
                     "два Красноярска в «Организациях» это дубль в любом случае.",
