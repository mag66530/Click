...
# ════════════════════════════════════════════════════════════════════
#  Жива ли карточка – для сверки статусов КП
# ════════════════════════════════════════════════════════════════════

COMPANY_STATE_WAIT_MS = 20_000     # удалённая карточка видна сразу, живой хватает с запасом


def company_state(page: Page, gis_url: str) -> dict:
    """
    Открыть кабинет и ответить, что с карточкой: живая или удалена.

    Возвращает {'state': 'alive' | 'deleted' | 'login' | 'error', 'note': str}.

    Смотрим ровно то, что видит человек на главном экране кабинета: удалённая
    карточка встречает надписью «Компания … удалена из справочника 2ГИС» и
    кнопкой «Вернуть компанию в 2ГИС» – признак тот же, что в актуализации.

    Ждать здесь надо меньше, чем в актуализации: у той критерий готовности –
    «мы на странице „Данные о компании“», а экран удалённой карточки такой
    страницей не является, и полное ожидание превращалось бы в полминуты
    на каждый удалённый город. Поэтому свой цикл: «удалена» – выходим сразу,
    иначе ждём, пока страница устаканится.
    """
    url = build_company_url(gis_url)
    if not url:
        return {"state": "error", "note": "в ссылке из КП нет номера организации"}
    try:
        page.goto(url, wait_until="domcontentloaded", timeout=40_000)
    except Exception as e:  # noqa: BLE001
        return {"state": "error", "note": f"страница не открылась: {yb._short_error(e)}"}
    if looks_like_login_page(page):
        return {"state": "login", "note": "открылась страница входа – сессия 2ГИС не действует"}

    look, same, was = {}, 0, -1
    deadline = time.time() + COMPANY_STATE_WAIT_MS / 1000
    while time.time() < deadline:
        fresh = look_at_page(page)
        look = fresh or look
        if look.get("deleted"):
            return {"state": "deleted", "note": "кабинет пишет «удалена из справочника 2ГИС»"}
        size = int(look.get("size") or 0)
        same = same + 1 if size == was and size > 200 else 0
        was = size
    ...
    if looks_like_login_page(page):
        return {"state": "login", "note": "открылась страница входа – сессия 2ГИС не действует"}
    if not look:
        return {"state": "error", "note": "страницу кабинета прочитать не вышло"}
    if look.get("onCompany"):
        return {"state": "alive", "note": "страница «Данные о компании» открывается"}
    # Кабинет живой карточки может встретить и другим разделом (дашбордом),
    # если ссылка в КП ведёт на него. Раз мы внутри кабинета этой организации
    # и надписи об удалении нет – карточка живая.
    if "/orgs/" in (look.get("path") or ""):
        return {"state": "alive",
                "note": f"кабинет открывается («{look.get('title') or 'без заголовка'}»)"}
    return {"state": "error",
            "note": f"открылась не страница кабинета, а «{look.get('title') or '?'}» "
                    f"({look.get('path') or 'адрес неизвестен'})"}
