...
def _crosspost_report_block(project_id: str, posts: list[dict], state: dict) -> None:
    ...
    Состояние читаем ЗАНОВО с диска, а не берём снимок из начала прорисовки.
    Заказчица: «отчёт сразу не обновляется, приходится перезагружать
    страницу». Формирование идёт долго (браузер, десятки секунд), и снимок
    state, снятый до него, к моменту показа отчёта уже устарел. Свежее
    чтение стоит одного открытия файла и всегда показывает последнее.
    """
