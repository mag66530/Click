...
KNOWN = (
    ...
    ("tg_proxy", "Телеграм: прокси (если ТГ заблокирован)",
     "Только для Телеграма. Формат socks5://user:pass@host:port или http://host:port"),
...
# Значения по умолчанию, ОБЩИЕ для всех, кто пользуется Click. Локальная
# запись в файле настроек (или своё значение на конкретной машине) их
# перекрывает. Здесь – прокси для Телеграма: у части провайдеров ТГ
# заблокирован, и чтобы он работал У ВСЕХ без ручной настройки, прокси
# зашит сюда. Сменить прокси – поменять ОДНУ строку здесь.
#
# Внимание: это доступ к прокси, и он лежит в репозитории. Используйте
# отдельный/сменяемый прокси и меняйте пароль здесь при необходимости.
DEFAULTS = {
    "tg_proxy": "http://Alexandra:x5sale3adip2@205.172.57.182:1080",
}


def get(name: str) -> str:
    """
    Значение ключа: сначала из файла настроек этой машины, иначе – общий
    умолчательный (DEFAULTS). Пусто – ключа нет нигде.
    """
    v = (load().get(name) or "").strip()
    return v or (DEFAULTS.get(name) or "").strip()
