"""
crosspost_probe.py — первая проба API кросспостинга. Пока только ВКонтакте.

Зачем. Прежде чем встраивать кросспостинг в Click, надо один раз убедиться
живьём: токен работает, отложка создаётся, пост появляется в кабинете
сообщества как «отложенная запись». Этот скрипт делает ровно ОДИН такой пост
и больше ничего — потрогать API руками, не трогая приложение. Заодно это
живой пример того, как устроен вызов API (ответ на «не понимаю, как это работает»).
