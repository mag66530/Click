...
# Кого планировщик НЕ публикует в час выхода. Пусто – работают все, как было.
#
# Сейчас здесь Телеграм и МАКС-через-бота (13.08.2026, по решению заказчицы):
# отложки у ботов нет, время держал сам Click, и в назначенный час он отправлял
# пост – а раздел писал «вышло». Пост при этом уходил не в том виде, поэтому
# публикация выключена целиком, пока её не сделают по-настоящему. Родных
# отложек ВК, ОК и МАКС через веб это не касается: их держат сами площадки, и
# планировщик к ним не притрагивается. ЯБ по расписанию тоже работает.
PUBLISH_OFF = ("tg", "max")
    ...
    try:
        ...
        for project_id in projects_with_tasks():
            ...
            for task in tasks:
                ...
                # Публикация в час выхода выключена (13.08.2026, по решению
                # заказчицы). Телеграм в этом виде не готов: пост уходил не
                # тем видом, а раздел писал «вышло» – и это была неправда.
                # Уже стоящие задания не отправляем, а честно отменяем: иначе
                # они выстрелят в назначенный час у первого же, кто откроет
                # Click. Родные отложки ВК, ОК и МАКС этого не касаются –
                # их держат сами площадки, планировщик к ним не причастен.
                if family in PUBLISH_OFF:
                    task["state"] = "cancelled"
                    # Состояние поста возвращаем в «не сформировано»: Click с
                    # ним ничего не сделал, и говорить «запланировано» нельзя.
                    _record(project_id, task, cps.WAITING,
                            error="публикация Click в час выхода выключена")
                    log(f"{project_id} {task['id']}: не публикую – "
                        f"{cps.network_ru(task.get('network', ''))} выключен")
                    dirty = True
                    changed += 1
                    continue
