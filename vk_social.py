...
def _set_schedule(page, when: datetime, log: Callable[[str], None]) -> None:
    ...
    # скрытым, так что тревога могла быть и ложной. Ждём скрытия и даём ВК
    # время на ответ: 800 мс наугад для медленной сети было мало в любом случае.
    try:
        ...
        # Попап действительно остался. Значит ВК чем-то недоволен – и почти
        # наверняка написал чем. Без этого «не сработало» остаётся тупиком:
        # причина видна только глазами на снимке, а снимок доходит не всегда.
        raise RuntimeError(
            "ВК не принял отложку: попап планирования остался открытым. "
            ...
            + " Проверьте «Отложенные записи» сообщества: если запись всё же "
              "появилась, формировать заново не нужно – будет дубль") from e
