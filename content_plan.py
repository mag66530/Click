...
def _split_images(cell: str) -> list[str]:
    """
    Ссылки на фото из ячейки «Фото». Учитываем два нюанса реестра.

    1. Пометки. Для «Отгрузки» в ячейке ДВА адреса: «ЛОГО: <url>» и «БЕЗ ЛОГО:
       <url>» (картинка с логотипом на ней или без) – постим вариант С
       логотипом. У части постов вместо этого «СОЦ: <url>» (для соцсетей) и
       «ЯБ: <url>» (для Яндекс.Бизнеса) – для кросспостинга берём СОЦ. Нет
       пометок – все адреса подряд. «ЛОГО»/«СОЦ» ищем по-русски, latin «logo»
       в имени файла отбор не путает.
    2. Гиперссылки. Адрес в ячейке фото часто ГИПЕРССЫЛКА, и общее чтение
       реестра (_google_cell_markup) оборачивает её в [текст](адрес). Прежний
       поиск «кусок начинается с http» такой адрес НЕ видел, и фото не
       подхватывалось вовсе (реестр СМУ, 21.08.2026). Теперь берём КАЖДЫЙ
       http-адрес – и в анкоре `[..](url)`, и голый.
    """
    text = (cell or "").strip()
    if not text:
        return []
    ...
    low = "\n".join(lines).lower()
    if "лого" in low:                       # «ЛОГО» есть → только с логотипом
        lines = [ln for ln in lines
                 if "лого" in ln.lower() and "без" not in ln.lower()]
    elif "соц" in low:                      # «СОЦ/ЯБ» → для соцсетей берём СОЦ
        lines = [ln for ln in lines if "соц" in ln.lower()]
    seen: set[str] = set()
    out: list[str] = []
    for ln in lines:
