...
def _set_schedule(page, when: datetime, log: Callable[[str], None]) -> None:
    ...
    # Ждём, пока кнопка ПЕРЕСТАНЕТ БЫТЬ ВИДНОЙ, а не пока исчезнет из DOM.
    # Здесь стояло `.count()` – счёт узлов в разметке, без разницы, видны они
    # или нет. ВК закрывает календарь анимацией и какое-то время держит узел
    # скрытым, а иногда оставляет его насовсем. Отложка при этом создавалась,
    # но Click объявлял «попап не закрылся – похоже, не сработало», писал
    # посту «ошибка» и предлагал сформировать заново – то есть звал поставить
    # ВТОРУЮ отложку поверх уже стоящей. Ждём скрытия и даём ВК время на
    # анимацию: 800 мс наугад для медленного ответа сети тоже было мало.
    try:
        page.wait_for_selector(SEL["postponed_confirm"], state="hidden", timeout=8_000)
    except Exception as e:  # noqa: BLE001 – наружу уходит понятной строкой
        raise RuntimeError(
            "Попап планирования не закрылся после подтверждения – похоже, не сработало. "
            "ПРОВЕРЬТЕ «Отложенные записи» сообщества: если запись там уже есть, "
            "формировать заново не нужно – будет дубль") from e
